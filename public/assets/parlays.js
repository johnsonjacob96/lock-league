...
function renderParlayHall() {
 ...
 const card=({slip:s,miss,profit})=>{
  ...
  return `<article class="parlay-shame-card"><div class="parlay-shame-story"><p class="parlay-shame-kicker">PARLAY KILLER</p><h3>${escapeHtml(member?.name||'Mystery picker')}</h3><p class="parlay-shame-verdict">Every other leg hit. Let that sink in.</p><p class="parlay-shame-context">${escapeHtml(s.night)} · Week ${s.week} · ${s.legs.length-1} of ${s.legs.length} hit</p><div class="parlay-shame-marks" aria-label="${s.legs.length-1} legs hit, one leg missed">${s.legs.map(l=>`<span class="${l.result==='L'?'miss':'hit'}" aria-hidden="true">${l.result==='L'?'✕':'✓'}</span>`).join('')}</div><p class="parlay-shame-bet"><span>The crime scene</span>${escapeHtml(parlayLegLabel(miss))}</p><p class="parlay-shame-context">${escapeHtml(s.title)} · ${escapeHtml(s.game_key.replace('@',' @ '))}${member?'':' · Assign this leg to name the culprit.'}</p></div><div class="parlay-shame-receipt"><span class="parlay-shame-stamp">ONE MISS. EVERYBODY’S PROBLEM.</span><p class="parlay-shame-kicker">THE DAMAGE</p><strong class="parlay-shame-amount">${money}</strong><p>${profit==null?'Add the combined odds to calculate the winnings that got away.':`in group winnings killed<br><span>$5 × ${americanOdds(s.odds)} odds · profit only</span>`}</p><span class="parlay-shame-fine">The group chat is waiting for your apology.</span></div></article>`;
 };
...
function renderParlaySlip(s) {
 ...
 const photo=!['manual','game_total'].includes(l.market)?slPlayerPhoto(l.player):null;
...
function bindParlays() {
 ...
 document.querySelectorAll('[data-field="market"]').forEach(el=>el.onchange=()=>{readParlayDraft();const l=parlayState.draft.legs[Number(el.closest('[data-leg-index]').dataset.legIndex)];if(l.market==='game_total'){l.player='Game total';if(!['over','under'].includes(l.side))l.side='over';}else if(l.player==='Game total')l.player='';if(l.market==='anytime_td'){l.side='yes';l.line=null;}else if(l.side==='yes')l.side='over';paintParlays();});
