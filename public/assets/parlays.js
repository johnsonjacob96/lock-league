...
function parlayLegLabel(l) {if(l.market==='spread')return `${l.player} · ${l.line>0?'+':''}${l.line} Spread`;if(l.market==='game_total')return `Game total · ${l.side==='under'?'Under':'Over'} ${l.line}`;return l.market==='manual'?l.player:`${l.player} · ${l.market==='anytime_td'?'Anytime TD':`${l.side==='atleast'?`${l.line}+`:l.side+' '+l.line} ${PARLAY_MARKETS[l.market]||l.market}`}`;}
...
function renderParlaySlip(s) {
 ...
 const photo=!['manual','game_total','spread'].includes(l.market)?slPlayerPhoto(l.player):null;
...
function readParlayDraft() {
 ...
 s.count_reviewed=!!form.querySelector('#parlay-count-reviewed')?.checked;
 ...
 return {source_box:original.source_box,source_text:original.source_text,source_crop:original.source_crop,review:original.review,reviewed:!!el.querySelector('[data-field="reviewed"]')?.checked,player:v('player'),market:v('market'),side:v('side'),line:v('line')===''?null:Number(v('line')),member_id:v('member_id')===''?null:Number(v('member_id'))};
 });
}
