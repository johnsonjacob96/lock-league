...
export async function gradeCurrentWeeks(now = new Date()) {
  ...
  const notified = await notifyGradeResults(results).catch(() => false);
...
export async function notifyGradeResults(results, env) {
  const discord = env?.DISCORD_WEBHOOK_URL ?? globalThis.process?.env?.DISCORD_WEBHOOK_URL;
  const groupme = env?.GROUPME_BOT_ID ?? globalThis.process?.env?.GROUPME_BOT_ID;
