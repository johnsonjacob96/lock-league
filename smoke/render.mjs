...
import {loadChromium as chromium} from './playwright.mjs';
    ...
    for (const vw of [1280, 1024, 390]) {
      ...
      const r = await page.evaluate(async (mk) => {
        ...
        try {
          ...
          superLockState.markets = mk; superLockState.gameKey = "New England Patriots@Seattle Seahawks";
          ...
          for (const m of mk) for (const pl of m.players) {
            for (const side of (m.kind === "yes" ? [""] : ["over", "under"])) {
              superLockState.draft = { market: m.market, player: pl.player, side, line: null };
              const html = slBrowsePropsHtml();
              if (typeof html !== "string" || !html) throw new Error("empty picker html for " + m.market);
            }
            if (pl.alts && pl.alts.length) { superLockState.draft = { market: m.market, player: pl.player, side: "over", line: pl.alts[0].line }; slBrowsePropsHtml(); }
          }
          // Mount one and measure horizontal overflow in the narrow sidebar.
          superLockState.draft = { market: "receptions", player: "Cooper Kupp", side: "over", line: null };
          ...
          out.checks.pickerLaunch = !!document.querySelector("#sl-open");
