...
export function validateSlip(body, memberIds) {
 ...
 const legs=body.legs.map(l=>{
  if(typeof l.player!=='string'||!l.player.trim()||l.player.length>150)throw Error('Enter the player or pick for every leg.');
  if(!PROP_DEFS[l.market]&&l.market!=='manual')throw Error('Choose a supported market or Custom.');
  ...
  return {player:l.player.trim(),market:l.market,side:l.side,line:['anytime_td','manual'].includes(l.market)?null:l.line,member_id:l.member_id??null,result:null};
 });
...
export function legProgress(leg, summary, final=false) {
...
export async function refreshParlays(env,season,week,events) {
 ...
 for(const key of new Set(rows.map(r=>r.game_key))) {
  ...
  for(const row of rows.filter(r=>r.game_key===key)) {
   const legs=row.legs.map(l=>({...l,...(l.result?{}:legProgress(l,gp,final))}));
