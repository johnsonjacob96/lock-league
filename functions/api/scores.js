...
// Free, no key; cached 10s per warm isolate. Powers the LIVE pills and
...
export async function onRequestGet(context) {
  ...
  if (cache.data && cache.key === key && Date.now() - cache.ts < TTL_MS) {
    return json(cache.data, { headers: { "X-Cache": "HIT", "Cache-Control": "no-store" } });
  }
  try {
    ...
    return json(data, { headers: { "X-Cache": "MISS", "Cache-Control": "no-store" } });
  } catch (e) {
